 ...
 *
 * Android always asks the user the first time. On Android 12+, once Point has installed
...
final class ApkInstaller {
    ...
    static final String ACTION_STATUS = "app.point.INSTALL_STATUS";
    ...
    /**
     * False when the user first has to allow Point to install apps; the matching settings
        ...
        try {
            ...
            try (PackageInstaller.Session session = installer.openSession(sessionId)) {
                try (InputStream in = c.getInputStream();
                     OutputStream out = session.openWrite("point.apk", 0, length)) {
            ...
            listener.onMessage("Could not download the update. Are you online?");
        }
    }
            ...
            case PackageInstaller.STATUS_PENDING_USER_ACTION:
                ...
                if (confirm != null) {
                    try {
                        ...
                        listener.onMessage("Could not open the installer");
                    }
                }
                break;
            case PackageInstaller.STATUS_SUCCESS:
                listener.onMessage("Update installed");
                break;
